# ================================================
# static_hash_store package – mini "Static Hash Store" (SHS)
# ================================================
# 📂 File layout (single‑file representation for canvas; split into modules on disk):
# ├── static_hash_store/
# │   ├── __init__.py
# │   ├── store.py          ← core mmap+struct reader / locked writer
# │   ├── bloom.py          ← simple Bloom‑filter implementation
# │   ├── compression.py    ← delta‑encoding + zstd helpers
# │   └── const.py          ← magic numbers, version, struct formats
# ├── examples/
# │   └── build_store.py    ← CLI utility to build / append
# └── setup.py
#
# For brevity everything is embedded below; copy each section into its file.
# Requires: Python ≥3.9, numpy, zstandard (``pip install numpy zstandard``)
...
class Bloom:
    """ Minimal Bloom filter backed by ``bytearray``. """
...
def delta_encode(sorted_ints:list[int]) -> bytes:
    """Encode increasing ints as varint deltas."""
    out = bytearray()
    prev = 0
...
# ==================================================
""" Example: build store from integers 0..N‑1 """
